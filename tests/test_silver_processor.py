...
@pytest.fixture
def mock_spark():
    """Fixture para simular la sesión de SparkSession en la capa Silver."""
    spark = MagicMock()
...
def test_silver_processor_initialization(mock_spark):
    """Verifica que SilverProcessor configure correctamente las optimizaciones de Spark."""
    processor = SilverProcessor(mock_spark)
    assert processor.spark == mock_spark


def test_process_silver_weather(mock_spark):
    """Verifica el flujo de procesamiento de datos meteorológicos en Silver."""
    processor = SilverProcessor(mock_spark)
    ...
    # Configurar operadores de comparación lógica sobre el MagicMock
...
def test_process_silver_osm_roads_includes_geometry(mock_spark):
    """Verifica que el procesamiento de OSM en Silver incluya la geometría y centroides."""
    processor = SilverProcessor(mock_spark)
